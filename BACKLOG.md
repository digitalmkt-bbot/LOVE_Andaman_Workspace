...
- **Phase 2** — per-trip rate resolver `bookingV2ResolveRateType(agentId, routeId, travelDate)` + `bookingV2GetRTForTrip` wired into `bookingV2TripSubtotal` + FOC calc. Promo overlays Main by **travel date**, **per route**, `priority` desc; falls back to Main when no promo (verified: no behavior change until a promo exists). Commit `c905ecb`.
...
- **Resolve** button → choose one of two, routing into the EXISTING flows:
  - **Set new date** → `bookingV2RescheduleBooking(...)` (fee on own invoice, ghost+badge as today)
...
**Status:** Built · all 4 outcomes · verified. **Decision comes from ops** (Boat Operation cell popover → `bookingV2WeatherMark` → note popup → `bookingV2WeatherMarkConfirm` records closure in `SB_WEATHER_CLOSURES`/`sb_weather` + tags affected bookings · status change shows that day in Boat Op heatmap as `⛈ CANCEL` red tint). **Resolution happens in By-trip-date** — the cancelled trip shows a faint-red wash over the whole block (`.t2-trip-wx`) + a prominent alert banner with the note + "เริ่มแก้ by booking" button → opens `bookingV2WeatherPanel`.
...
- **Phase 1 · แจ้ง agent** (`bookingV2WeatherNotify`) — staff notifies the agent; status `awaiting → notified` (+ timestamp). Agent then talks to the customer offline.
- **Phase 2 · ดำเนินการ** (`bookingV2WeatherResolveOne`) — once the agent replies, pick the per-booking outcome: **reschedule** (move `trip.date` + `rebook` log · booking stays `confirmed`) · **refund** (`cancelled_weather` + negative refund payment + void invoice) · **เก็บเครดิต** (paid → `acctCreateDeposit` + void invoice) · **ยกเลิก** (void unpaid invoice). status → `resolved`.
- Panel header shows progress counts (รอแจ้ง / แจ้งแล้ว-รอตอบ / จัดการแล้ว). Resolved bookings stay listed (✓ outcome) so the board is auditable. Bookings tagged via `bookingV2WeatherTagBookings` on closure + panel-open (excludes charter). `getSeatsConsumed` excludes `cancelled_weather` → seats auto-released. Backups: `pre_weather_cancel`, `pre_weather_note`, `pre_wx_workflow` (all 20260603).
...
- **By trip · date — sort by Zone:** manifest already *groups* by zone (zoneBlocks, ordered by `bookingV2ZoneOrder`). If "sort" means ordering rows within a zone (e.g., by pickup time) or a flat zone-sorted list instead of grouped — confirm preference, then small change in `bookingV2RenderTab2`.
...
**Trigger:** booking BK-26060015 (Traveloka/OTA) saved with **no pickup** (hotel/area/time blank). Confirmed NOT a bug — every other field saved fine; the form only hard-requires Agent (`if(!d.agentId) alert...` in `bookingV2CommitBooking`). Pickup was simply never entered (typical OTA: hotel unknown at booking time).

**Two enforcement levels to mix:**
- **Hard block** — incomplete = can't save (`alert` + `return` in `bookingV2CommitBooking`, like the existing agent check).
...
**Where to build:** `bookingV2CommitBooking` (~line 44954) — add a validation block after the existing agent check; collect missing-required → block; collect missing-soft → `confirm` + tag `bk.incomplete=[...]`. Manifest row (`bookingV2RenderTab2`) → render a ⚠ chip when `bk.incomplete?.length`.
...
When zone = No Transfer (`NoTransfer`/`NT`) the Hotel/Pickup field is no longer required — label flips to **"Note · optional · self-arrival"** with an optional-note placeholder, and the `bookingV2CommitBooking` hard-block (`Hotel / pickup location`) is skipped (detected via area.zone / `pickupZoneFilter` / trip zone = No Transfer). Other zones (PK/KL) still hard-require the hotel. Form flag `_pickIsNT`. Backup: `BACKUP/allotment_v2_20260604_pre_notransfer_pickup.html`.
...
**Phase 1 · Categorized reason — ✅ DONE (2026-06-07).** Cancel modal (`bookingV2CancelModal`) now has a **Reason dropdown** (`#bkc-cat`) from `BKV2_CANCEL_REASONS` (stable `code` + Thai label + `group:customer|operator|other` + `def` charge suggestion). Picking a reason auto-sets the suggested charge radio (`bookingV2CancelPickReason`, user can override). Note textarea is now optional (required only for `other`). Stored on `bk.cancellation = {category, categoryLabel, group, note, reason, chargeType, chargeAmount, at, by}` + `bk.cancelCategory`. Reasons: `customer_cancel · no_show · sick · flight_visa · agent_error` (customer) · `operator · force_majeure` (operator) · `other`. Weather stays its own flow (`cancelled_weather`). Backup: `BACKUP/allotment_v2_20260607_pre_cancel_reasons.html`.

**Phase 2 · Partial cancellation — ✅ DONE (2026-06-07).** **"− Reduce pax" button** in voucher detail (`bookingV2DetailPartial` · gated by `canCancel`). Modal `bookingV2PartialModal(bookingId,tripIdx)`: trip picker (multi-trip) → remove count per **present pax key** (`BKV2_PAX_KEYS` = ad/chd/inf/foc × base/_fr/_th) → reason (shared `BKV2_CANCEL_REASONS`) + note (required for `other`) + price-reduction/refund ฿. `bookingV2PartialConfirm` → `bookingV2PartialCancel(id,tripIdx,{removed,totalRem,category,note,refundMode,refund})`: decrements `trip.pax[k]` (seats auto-released via `getSeatsConsumed`), reduces `bk.priceBreakdown.total`+`bk.total` by refund, pushes `bk.partialCancels[]={date,tripIdx,paxRemoved,count,category,categoryLabel,group,note,refundMode,refund,at,by}`, history tag `Cancel`, persists `sb_bookings`. Booking stays `confirmed`. **Money is an explicit radio (`bkp-money`): `none` = ไม่คืนเงิน (เก็บราคาเดิม · refund 0) · `refund` = คืนเงิน/ลดราคา (amount required)** → stored as `refundMode`. ⚠ Does NOT auto-recompute the full seat quote (price stored, not live). ⚠ If already invoiced, invoice not auto-adjusted (manual). **Folded into the Phase-3 report (2026-06-07):** report aggregates `partialCancels` into Pax cancelled (full vs partial sub), By reason/Fault/Month/Agent, and no-show; KPI "Cancellations" shows full count + "+N partial".

**Phase 3 · Cancellation report — ✅ DONE (2026-06-07).** New **"Cancellations" tab** in Booking module (`_bkV2.tab==='cancel'` · `bookingV2RenderCancelReport()` · wired in `bookingV2RenderTopbar`/`bookingV2RenderTabBody`/`bookingV2TopbarMeta`). KPIs: cancellations + rate, pax cancelled, **no-show rate**, fees collected. Sections: By reason (bars + customer/operator/other tag + count·pax·fee), Fault side split bar, Top cancelling agents, By month bars. `cancelled_weather` auto-counts as category `weather` (operator). Reads `bk.cancelCategory`/`bk.cancellation`. Empty state when none. Backup: `BACKUP/allotment_v2_20260607_pre_cancel_report.html`.
...
- `bookingV2SubmitBooking` (~46485): if `quote.totalFoc > 0` → commits as **`pending_foc`** (awaiting approval), NOT `confirmed`. Free seats must be approved first. Only **`bookingV2FocApprove`** (~47073) sets `bk.status='confirmed'` (via the FOC Approval panel · Approve button). So FOC bookings sit in Pending FOC until someone clicks Approve.
- Quote vs Pending FOC: `bookingV2SaveQuote` saves as `quote` (grey); Submit saves as `pending_foc` (amber) / `confirmed`. `bookingV2Norm` maps `status==='pending'`→display `quote`.
...
- Pay column chip `bookingV2CotChip` (cyan "COT ฿X") from `bk.cashOnTour` (or a "cash on tour …" note line).
...
**Where:** the family-card boat grid + the per-variant trip card (`bookingV2RenderTab2` · the `.t2-vcard`/`.t2-vbrow` per-boat rows + the `agg[boatId]` aggregation that today only sums pax + guide langs + special meals).
...
4. พิมพ์ใบงานรถ (ต่อยอด `bookingV2VanJobOrder` เดิม 2 ภาษา) + โผล่ใน Transfer Fleet "Daily"
...
## ⏸ "เรือไม่ได้ออก" — ควรอ่านจากการเช็คอินหน้าท่า ไม่ใช่เดาจากใบจอง (พบ 2026-08-31 · ทำไปครึ่งทาง)

**สถานะ:** §pxNoSail ทำแล้วและใช้งานอยู่ (Trip P&L · Daily / Monthly / Analysis)
แต่กติกาที่ใช้ตัดสินยัง **เดา** อยู่ · ของที่ควรทำต่อคือเปลี่ยนไปอ่านข้อเท็จจริงแทน

**ที่มา:** หน้า Trip P&L วันที่ 30 ส.ค. 2026 · Aluminous2 เส้น Whale Shark Sunset
ขึ้น 0 คน · 0 ใบจอง · ไม่ได้เช็คอิน แต่โดนต้นทุน ฿32,764 ทำให้ทั้งวันขาดทุน −฿25,431
สาเหตุ: `pxDay()` หยิบ **ทุกลำที่มีเส้นทางอยู่ใน `TRIPS[date]`** มาคิดต้นทุน
ซึ่งแปลว่า "จัดเรือไว้บนกระดาน" ไม่ใช่ "เรือออก" — สองอย่างนี้ไม่เหมือนกัน
และสูตรต้นทุนส่วนใหญ่ไม่ผูกกับจำนวนคน (ค่าเสื่อม · กัปตัน · เด็กเรือ · ไกด์ · ค่าเทียบท่า ·
น้ำมันที่คิดจากชั่วโมงเครื่อง) ลำที่ไม่มีใครขึ้นจึงยังโดนชาร์จเต็มจำนวน

ย้อนทั้งฐานข้อมูลเจอ **32 ลำ** แบบนี้ · **19 ลำเป็น Aluminous2 เส้น Whale Shark Sunset เส้นเดียว**
(เส้นนี้ถูกวางบนกระดานเกือบทุกวันแต่ขายไม่ค่อยออก — เป็นปัญหาการจัดตารางด้วย ไม่ใช่แค่บั๊ก)
เดือน ก.ค. เดือนเดียวมี 18 ลำ รวม **฿466,683** ดัน cost ratio ของเดือนจาก 89% เป็น 98%

**ที่ทำไปแล้ว (§pxNoSail · `pxTrip` / `pxDaily` / `pxDetail`):**
- กติกาปัจจุบัน: `PX.tot === 0 && PX.bk === 0 && !A.ran` → `noSail` → ทุกบรรทัดใช้ 0 · `cost = 0` · `st = 'nosail'`
  (`est` ของทุกบรรทัดยังอยู่ครบ ไม่ได้ลบ จึงยังเปิดดูได้ว่า "ถ้าออกจะเท่าไหร่" = `t.wouldCost`)
- ทางกลับ: ปุ่ม `pxRan(bid)` ยืนยัน "ออกจริง" เก็บเป็น `A.ran` ใน `trip_actuals`
  (ที่เดียวกับการปิดยอด) เผื่อเรือออกไปส่งของ · กลุ่มยกเลิกหน้างานหลังเติมน้ำมันแล้ว · เรือสำรองที่ต้องวิ่งตาม
- ตัวนับ `nProfit` / `nLoss` / `nReal` / `nNoPlan` / `nNoFuel` และ Data confidence ตัดลำที่ไม่ได้ออกออก
  ไม่งั้นกำไร 0 บาทถูกนับเป็น "ทริปที่ทำกำไร" และตัวหารเกินจริง
- ผลย้อนหลัง cost ratio: มิ.ย. 91→90% · ก.ค. 98→89% · ส.ค. 89→88%

**สิ่งที่ยังค้าง — เปลี่ยนจาก "เดา" เป็น "ข้อเท็จจริง":**
เงื่อนไข `PX.bk === 0` เป็นการ **อนุมาน** ว่าไม่มีใบจอง = เรือไม่ได้ออก ซึ่งผิดได้สองทาง:
1. **False positive** — เรือออกจริงแต่ไม่มีใบจองผูกกับลำนั้น (ไปส่งของ · วิ่งสำรอง ·
   ใบจองถูกย้ายลำหลังเรือออกไปแล้ว) → ตอนนี้ต้องให้คนไปกดปุ่ม "ออกจริง" เอง
2. **False negative** — มีใบจองอยู่แต่ลูกค้าไม่มาทั้งลำและเรือก็ไม่ได้ออก
   → ตอนนี้ยังคิดต้นทุนเต็ม เพราะ `bk > 0`

ทางที่ตรงกว่าคือผูกกับ **การเช็คอินหน้าท่า** — เรือที่ไม่มีการเช็คอินเลยในวันนั้น = ไม่ได้ออก
เพราะการเช็คอินเป็นสิ่งที่ "เกิดขึ้นจริงหน้าท่า" ไม่ใช่สิ่งที่เราวางแผนไว้ล่วงหน้า
ต่างจากใบจองซึ่งเป็นความตั้งใจ ไม่ใช่ผลลัพธ์

**Scope ที่ต้องทำ:**
1. หาตัวชี้ขาดที่เชื่อได้จริงก่อน — `PIER_JOB` / `pjOf(date,bid)` (ใบงานเรือประจำวัน) และสถานะเช็คอิน
   ในหน้า "เช็คอินหน้าท่า" · ต้องตรวจก่อนว่ามีข้อมูลย้อนหลังครบพอไหม
   ⚠ ถ้าย้อนหลังไม่ครบ (เริ่มใช้ทีหลัง) จะทำให้วันเก่า ๆ กลายเป็น "ไม่ได้ออก" ทั้งหมด — ต้องมีวันตัด
2. ลำดับความเชื่อ (มากไปน้อย): `A.ran` ที่คนกดเอง → มีการเช็คอิน → มีของจริงลงแล้ว
   (น้ำมันในใบงานเรือ / ค่าอาหารร้าน / ค่ารถ) → มีใบจอง · ถ้าไม่เข้าข้อไหนเลย = ไม่ได้ออก
   *(ข้อ 3 สำคัญ: ถ้ามีน้ำมันลงในใบงานเรือแล้ว แปลว่าเรือติดเครื่องจริง ต่อให้ไม่มีใครขึ้น)*
3. เปลี่ยน `noSail` ใน `pxTrip` ให้ใช้ลำดับข้างบน · UI เดิม (ป้ายเทา · ปุ่ม "กดถ้าออกจริง" ·
   `wouldCost`) ใช้ต่อได้ทั้งหมด ไม่ต้องแก้
4. เพิ่มปุ่มตรงข้าม — "ไม่ได้ออก" สำหรับลำที่มีใบจองแต่เรือไม่ได้ออกจริง (แก้ false negative ข้อ 2)

**เกี่ยวข้องกัน:** เส้น Whale Shark Sunset ถูกวางบนกระดานถี่เกินจริง (19 วันที่ไม่มีคนเลย)
เรื่องนี้แก้ที่หน้า Boat Operation ไม่ใช่ที่ P&L — แต่ควรดูพร้อมกัน

**Workaround จนกว่าจะทำ:** กติกาปัจจุบันครอบเคสส่วนใหญ่ได้แล้ว · เคสที่หลุดให้กดปุ่ม
"ไม่ได้ออก · กดถ้าออกจริง" บนการ์ดของลำนั้นในหน้า Trip P&L Daily (สิทธิ์ `accounting`)
