...
**Two stacks in one repo (since 2026-09-08, when `refactor/frontend` merged into `lk-inbox`).** Everything above is the **monolith** — `server.js` at the repo root serving `allotment_v2/` — and it is still what staff use at `rsvn.loveandaman.com`. Alongside it now sits **`platform/`**, a separate pnpm workspace (Fastify API + Vite/React `ops-web`) that was scaffolded to replace the monolith by strangler migration. The two are joined by exactly two seams, both off by default:
...
**Active direction (2026-09-15): replacing `server.js` with `operation-backend`, not `platform/`.** `platform/` is being abandoned — its scaffold and the legacy `os-backend/src/mapping/` (field_mapping.json + os_repo.js, the JSONB-blob REST-index engine `/api/load`/`/api/save` lean on) are staged for deletion on `integration/operation-backend`. The real successor is a **separate repo**, [`operation_backend`](https://github.com/TheTaTha5/operation_backend) (Fastify + Postgres, deployed on Railway as `operationbackend-production.up.railway.app`), being tested against a sandbox copy of this frontend at `loveandamanworkspace-pano-frontend-production.up.railway.app` (NOT `rsvn.loveandaman.com` — that stays on `server.js` until this is proven). Reasoning: the monolith's whole-blob `/api/load`/`/api/save` JSONB sync is hard to maintain; `operation_backend` is a proper granular REST API (`/v1/routes`, `/v1/boats`, `/operations/deployments`, `/operations/allotment`, `/v1/bookings`, `/v1/seat-locks`) with real migrations and normalized tables (booking headers/passengers as columns, not blob). The plan is a **direct frontend rewrite** (`allotment_v2/js/01-auth-sync.js` and the booking/allotment call sites calling operation-backend's REST endpoints directly), not the `api-proxy.js` passthrough — `api-proxy.js` still exists and works as a stopgap, but is not the target architecture.

Coverage reality check before assuming any given screen is portable: as of 2026-09-15 operation-backend only implements **routes/boats catalogue, deployments, allotment/manifest/availability, bookings, and seat locks**. It has **no equivalent yet** for agents, rate types, invoices/payments/deposits/VAT/accounting, fleet (engines/gearboxes/maintenance/incidents/inventory/fuel), van jobs, pickup setup/zones, or market/demand data — those stay on `server.js`/the blob until built. Auth is also a gap: operation-backend expects `Authorization: Bearer <OIDC access token>` (Authentik, PKCE) per request; the frontend's PKCE login flow is not implemented yet (tracked as `todo/frontend-authentication.md` in the operation_backend repo) — without it every operation-backend endpoint except `/api/health` and the currently-unauthenticated `/v1/routes`/`/v1/boats` 401s. Confirmed by hitting `operationbackend-production` directly: `/v1/bookings`, `/operations/*`, `/v1/seat-locks` → 401 `Bearer access token required`; `/v1/routes`, `/v1/boats` → 404 on that deployment (stale build, not on the documented route set — verify against the repo's current `main` before assuming a 404 means "not implemented").
