...
- **Constraints/invariants:** No persistence/schema/SQL/API behavior change. No scratch Postgres available in this sandbox — DB-backed checks must be built into the CI workflow itself and reported as `not_run`/`blocked` locally, never `passed`. `esc`/`escapeHTML` and `bookingV2LocalYMD` conventions N/A (no HTML edits made).
