...
- **Requested outcome:** When a weather-cancelled trip is resolved with the "reschedule" outcome, the booking's boat/van/check-in assignment for the old day must be cleared, matching the behavior already implemented by `bookingV2RescheduleBooking` and the booking-edit date-change path, instead of silently carrying the previous day's boat and van onto the new date.
- **Acceptance criteria:**
  - `bookingV2WeatherResolveOne`'s `reschedule` branch clears the OLD date's ops (`boatId`, `vanId`, `vanReturnId`, `returnSameVan`, `vanGroup`, `vanSeq`, `vanSplits`, `pickupTimeFinal`, `vanCheckin`, `pierCheckin`) before moving the trip date, for both single-day bookings (`bk.ops`) and multi-day/OVN bookings (the matching `t.ops`), per `bkOpsRead`/`bkOpsFor`'s per-day semantics.
  - The fix reuses the existing `bkOpsClear(b, date, opts)` helper (added in commit `a747f42`, "Per-trip ops: boat/van assignment moves onto the travel day") rather than duplicating a third inline copy of the clearing logic already present in `bookingV2RescheduleBooking` (`:77572`) and the `bookingV2CommitBooking` edit-path date-change block (`:76886`).
  - No change to the non-reschedule outcomes (refund/credit/cancel) of `bookingV2WeatherResolveOne`.
  - `node --check` passes on the extracted main `<script>` after the edit.
- **Allowed scope:**
  - `allotment_v2/allotment_v2.html` (`bookingV2WeatherResolveOne` only, ~line 60024-60037)
...
- **Starting assumptions:**
  - `bookingV2WeatherTagBookings` only ever tags seat-mode trips (`t.bookingMode !== 'charter'` is excluded at `:59968`, per `allotment_v2/docs/workflows/01-booking-lifecycle.md` §3.7), so the weather-reschedule path never needs the charter "keep the boat" exception that `bookingV2RescheduleBooking` and the edit-path both carry for charter bookings.
...
|---|---|---|
| `allotment_v2/allotment_v2.html` | In `bookingV2WeatherResolveOne`'s `reschedule` branch (~line 60032-60037), call `bkOpsClear(bk, date)` on the OLD date before moving `trip.date` to the new date, guarded by `ndv!==date` so a no-op resolve (new date left equal to the old date) does not clear a still-valid assignment. | Yes |
...
| `git diff --stat` / `git diff -- allotment_v2/allotment_v2.html` | Passed — diff is a single 4-line insertion inside `bookingV2WeatherResolveOne`'s `reschedule` branch; no other lines touched. |
...
- **Decisions:**
  ...
  - Did not add the charter "keep the boat" exception that `bookingV2RescheduleBooking` and the edit-path both implement, because `bookingV2WeatherTagBookings` only tags seat-mode trips (charter trips are excluded at `:59968`), so that branch is unreachable from this code path.
- **Known risks:**
...
- **Follow-up work:**
  ...
  - Consider also routing `bookingV2RescheduleBooking`'s and the `bookingV2CommitBooking` edit-path's inline ops-clear blocks through `bkOpsClear()` for consistency, since both currently duplicate logic that now overlaps with the helper (out of scope for this ticket).
- **Rollback procedure:** `git revert` the LAM-18 commit on this branch (a single 4-line addition inside `bookingV2WeatherResolveOne`); no data migration or persisted-schema change to unwind, since the fix only changes in-memory mutation order of existing `bk.ops`/`t.ops` fields before the existing persistence calls run.
...
- **Unrelated changes left untouched:** None — the only implementation edit is the 4-line insertion inside `bookingV2WeatherResolveOne` described above.
